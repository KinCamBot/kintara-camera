(function () {
  ...
  function injectPatched(out) {
    function go() {
      var blob = new Blob([out], { type: 'text/javascript' });
      var url = URL.createObjectURL(blob);
      var s = document.createElement('script'); s.type = 'module'; s.src = url;
      // free the object URL once the module has loaded (or failed) so it does not leak for the page's lifetime
      s.onload = s.onerror = function () { try { URL.revokeObjectURL(url); } catch (e) {} };
      (document.head || document.documentElement).appendChild(s);
      whenReady();
    }
    // our content script runs very early, so make sure the page's importmap is in the DOM
    // before our module tries to resolve bare imports like 'three'.
    if (document.querySelector('script[type="importmap"]')) { go(); return; }
    var t = 0;
    var iv = setInterval(function () {
      if (document.querySelector('script[type="importmap"]')) { clearInterval(iv); go(); }
      else if (++t > 500) { clearInterval(iv); console.warn('[KinCam] no importmap after ~4s; injecting anyway (bare imports may not resolve).'); go(); }
    }, 8);
  }

  // game.js is blocked at the network level, so the original never runs (no race). Fetch it
  // ourselves, patch in the camera, and inject the patched module.
  fetch(location.origin + '/game.js', { cache: 'no-store' })
    .then(function (r) { return r.text(); })
    .then(function (code) {
      var out;
      try { out = absolutizeImports(patchGameSource(code)); }
      catch (e) {
        // patch failed (the game bundle changed) -> load the game UNMODIFIED so it still works,
        // just without the camera. The original is network-blocked, so we must re-inject it ourselves.
        console.warn('[KinCam] patch failed, loading the game unmodified: ' + e.message);
        try { out = absolutizeImports(code); } catch (e2) { console.warn('[KinCam] could not load original game code: ' + e2.message); return; }
      }
      injectPatched(out);
    })
    .catch(function (e) { console.warn('[KinCam] could not fetch game code (the game may not start): ' + e.message); });

  function whenReady() {
    var tries = 0;
    var iv = setInterval(function () {
      if (typeof window.__kintaraCameraMode === 'function') { clearInterval(iv); mountPanel(); }
      else if (++tries > 800) { clearInterval(iv); console.warn('[KinCam] camera engine did not initialize within ~80s; the panel will not appear. Try refreshing.'); }
    }, 100);
  }
